...
class Codevz_Plus {
	...
	public static function get_page_as_element( $id = '', $query = 0 ) {
		...
		if ( ! is_numeric( $id ) || $id === '404' ) {
			...
			if ( empty( $page->ID ) ) {
				$page = self::get_page_by_title( $id, true, 'elementor_library' );
			}
	...
	public function init() {
		...
		if ( function_exists( 'vc_add_shortcode_param' ) ) {
			...
			$elements = [ 'button', 'login_register', 'posts', 'social_icons', 'stylish_list', 'popup', 'working_hours', 'gallery', 'carousel', 'subscribe' ];
	...
	public static function maintenance_mode( $i ) {
		...
		// Custom page.
			...
			$mt = self::get_page_by_title( $mt, 'object', 'page' );
